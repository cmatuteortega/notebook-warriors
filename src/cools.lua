...
-- Rimmed in pale blue (`Sprites.rim`), which this is the only thing in the game
-- to wear and the reason is worth keeping: everything on this page is ink, and
-- an S is the same colour and the same weight of line as the hero, the crowd and
-- every mark you have left behind -- six thin strokes crossing a page made of
-- thin strokes. The rim is what lifts it off all that, and from across the page
-- you see the blue coming before you read the S.
--
-- Every rim goes down before any of the bodies, so that where two of them cross,
-- one S's rim can never sit on another's ink.
function CoolS:draw(game)
    local sprite = Sprites.cools

    for _, s in ipairs(self.live) do
        Sprites.rim(sprite, s.x, s.y)
    end
